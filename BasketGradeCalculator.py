        ...
        if item in transaction:
            ...
            if (len(list(set(hrtItem) & set(transaction)))>0):
                ...
                if (len(list(set(rtItem) & set(transaction)))>0):
                    grade = grade+((1/basketSize)*RTFactor);
                    rtFound = rtFound+1


    if (grade < 0.5 or bingoFound< 1 or ((bingoFound + hrtFound) < 2 and (bingoFound + rtFound) < basketSize)):
        grade = 0;
    return grade;
...
def CalculateBasketTotalGrade(basket,transactions):
    ...
    for transaction in transactions:
        totalGrade =  totalGrade + CalculateGradeForBasketInTransaction(basket, transaction)        
        i = i + 1
...
basketsFile = open("C:\\Dev\\Innovation\\customerSegmentation\\Data\\Items\\AllBasketsSortedBy3rdItemOccurences_Baskets5006-10005.csv", "r")
# basketsFile = open("C:\\Dev\\Innovation\\customerSegmentation\\Data\\Items\\AllBasketsSortedBy3rdItemOccurences.csv", "r")
...
def ScoringBasketBy20():
    allStartTime = time.time()
    gradesPerBasket = pd.DataFrame(columns = ['basket', 'grade'])
    basketsWithGradesDf = pd.DataFrame(columns=['BasketNumber', 'Index', 'TotalBasketScore', 'Item1', 'Item1Desc', 'Item2', 'Item2Desc', 'Item3'
    , 'Item3Desc', 'item3Occurences', 'item3Dist'])
    basketCounter=0
    ...
    for line in basketsLines.split("\n"):
        ...
        basket.append(lineArray[2])     
        basket.append(lineArray[4])     
        basket.append(lineArray[6])    
           
        grade = CalculateBasketTotalGrade(basket,allTransactions)            
        
        basketsWithGradesDf.at[basketCounter, 'BasketNumber'] = lineArray[0]
        basketsWithGradesDf.at[basketCounter, 'Index'] = lineArray[1]
        basketsWithGradesDf.at[basketCounter, 'TotalBasketScore'] = grade
        basketsWithGradesDf.at[basketCounter, 'Item1'] = lineArray[2]
        basketsWithGradesDf.at[basketCounter, 'Item1Desc'] = lineArray[3]
        basketsWithGradesDf.at[basketCounter, 'Item2'] = lineArray[4]
        basketsWithGradesDf.at[basketCounter, 'Item2Desc'] = lineArray[5]
        basketsWithGradesDf.at[basketCounter, 'Item3'] = lineArray[6]
        basketsWithGradesDf.at[basketCounter, 'Item3Desc'] = lineArray[7]
        basketsWithGradesDf.at[basketCounter, 'item3Occurences'] = lineArray[8]
        basketsWithGradesDf.at[basketCounter, 'item3Dist'] = lineArray[9]
        ...
        # print("basket", basketCounter)
        basketCounter = basketCounter + 1
        if (basketCounter % 100 ==0):
            print("basket", basketCounter, "before save...")
...
allTransactions = []
numberOfTransactionsToUse = 530000
count = 0
